...
```
backend: cd Backend && rvm 3.3.0@greenie exec rails server
frontend: cd Frontend && yarn serve
workers: cd Backend && rvm 3.3.0@greenie exec bundle exec sidekiq -C config/sidekiq.yml
cable: cd Backend && rvm 3.3.0@greenie exec ./bin/cable
mail: mailcatcher -f
```
