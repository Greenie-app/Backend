...
Greenie.app required Ruby 2.7.2, PostgreSQL, and Redis. (If you use Homebrew,
...
```
backend: cd Backend && rvm 2.7.2@greenie exec rails server
frontend: cd Frontend && yarn serve
workers: cd Backend && rvm 2.7.2@greenie exec bundle exec sidekiq -C config/sidekiq.yml
cable: cd Backend && rvm 2.7.2@greenie exec ./bin/cable
mail: mailcatcher -f
```
...
```
backend: cd Backend && rvm 2.7.2@greenie exec rails server -e cypress
frontend: cd Frontend && yarn run test:e2e
workers: cd Backend && redis-cli flushall && rvm 2.7.2@greenie exec bundle exec sidekiq -C config/sidekiq.yml -e cypress
cable: cd Backend && rvm 2.7.2@greenie exec ./bin/cable -e cypress
```
