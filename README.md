...
```
backend: cd Backend && rvm 3.4.5@greenie exec rails server
frontend: cd Frontend && yarn serve
workers: cd Backend && rvm 3.4.5@greenie exec bundle exec good_job start
anycable: cd Backend && rvm 3.4.5@greenie exec anycable
ws: cd Backend && rvm 3.4.5@greenie exec bin/anycable-go --port=8080
mail: mailcatcher -f
```
...
```
backend: cd Backend && rvm 3.4.5@greenie exec rails server -e cypress -b localhost
frontend: cd Frontend && yarn run test:e2e
workers: cd Backend && RAILS_ENV=cypress rvm 3.4.5@greenie exec bundle exec good_job start
anycable: cd Backend && RAILS_ENV=cypress rvm 3.4.5@greenie exec anycable
ws: cd Backend && rvm 3.4.5@greenie exec bin/anycable-go --port=8080
```
